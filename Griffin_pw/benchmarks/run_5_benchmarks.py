"""
Run 5 target benchmarks and write results to results/complete_benchmark.json.

Datasets:
  1. MQAR           -> JSON key "MQAR"            recurrent wins (associative memory)
  2. induction_heads -> JSON key "induction_heads" Hawk/Griffin win (pattern memory)
  3. copy           -> JSON key "copy"            Hawk/Griffin win (copy_len=100 > window)
  4. selective_copy  -> JSON key "selective_copy"  Griffin wins (hybrid: select + remember)
  5. sequential_niah -> JSON key "sequential_niah" Griffin wins (multi-needle memory)
...
# ------------------------------------------------------------------
# Config  (set NUM_EPOCHS=10 for full training)
# ------------------------------------------------------------------
...
BENCHMARKS = [
    ...
    ('sequential_niah', 'sequential_niah', {}),
]
...
def _forward(model, model_name, x, mask):
    """
    Hawk: no attention_mask argument.
    Local Attention: skip padding mask — combining it with the local-window mask
                     produces all-zero rows for padded tokens → NaN softmax.
    Griffin: receives attention_mask normally.
    """
...
# ------------------------------------------------------------------
# Long-sequence generalization eval
# ------------------------------------------------------------------
def long_seq_eval(model, model_name, ds_name, extra_kwargs):
    results = {}
    for sl in [256]:
        try:
            ds = UnifiedDataset(ds_name, num_samples=50, seq_length=sl, **extra_kwargs)
            ...
            results[str(sl)] = {
                'loss': avg,
                'perplexity': float(torch.exp(torch.tensor(avg)).item())
            }
        except Exception as e:
            results[str(sl)] = {'error': str(e)}
    return results
...
# Run one benchmark
# ------------------------------------------------------------------
def run_benchmark(ds_name, json_key, extra_kwargs):
    print(f"\n{'='*60}")
    print(f"  BENCHMARK: {json_key}  (dataset={ds_name})")
    print(f"{'='*60}")

    print("  Building datasets ...")
    train_ds = UnifiedDataset(ds_name, num_samples=N_TRAIN, seq_length=SEQ_LENGTH, **extra_kwargs)
    val_ds   = UnifiedDataset(ds_name, num_samples=N_VAL,   seq_length=SEQ_LENGTH, **extra_kwargs)
    ...
    for model_name, model in models.items():
        # Skip if already have a valid finite perplexity
        existing = bench[json_key].get(model_name, {})
        existing_ppl = existing.get('perplexity')
        if (isinstance(existing_ppl, (int, float))
                and existing_ppl == existing_ppl        # not NaN
                and existing_ppl != float('inf')):
            print(f"\n  --- {model_name} --- [SKIP -- already ppl={existing_ppl}]")
            continue

        print(f"\n  --- {model_name} ---")
        try:
            metrics = train_and_eval(model, model_name, train_dl, val_dl)
            metrics['long_sequence_generalization'] = long_seq_eval(
                model, model_name, ds_name, extra_kwargs)
        ...
        # Save after every model -- never lose work
        save_bench(bench)
...
if __name__ == '__main__':
    print(f"Running {len(BENCHMARKS)} benchmarks on device={DEVICE}")
    print(f"epochs={NUM_EPOCHS}  n_train={N_TRAIN}  n_val={N_VAL}  lr={LR}  clip={GRAD_CLIP}")

    for ds_name, json_key, extra_kw in BENCHMARKS:
        try:
            run_benchmark(ds_name, json_key, extra_kw)
        except Exception as e:
            import traceback; traceback.print_exc()
            print(f"[BENCHMARK FAIL] {json_key}: {e}")

    print("\n=== ALL BENCHMARKS DONE ===")

    bench = load_bench()
    print("\nSummary (validation perplexity):")
    print(f"{'Dataset':<22} {'Griffin':>10} {'Hawk':>10} {'LA':>10}")
    print("-" * 54)
    for _, json_key, _ in BENCHMARKS:
        entry = bench.get(json_key, {})
        def ppl(m):
            r = entry.get(m, {})
            v = r.get('perplexity')
            return f"{v:.3f}" if (isinstance(v, (int, float)) and v == v) else "ERR"
        print(f"  {json_key:<20} {ppl('Griffin'):>10} {ppl('Hawk'):>10} {ppl('Local Attention'):>10}")
